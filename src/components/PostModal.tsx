...
const PostModal = ({ open, onClose }: PostModalProps) => {
  ...
  const handlePost = () => {
    ...
    toast({ title: "Posted! 🎉", description: `Your ${selectedMood} post with ${parts.join(", ")} is live` });
    handleClose();
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-end justify-center" style={{ animation: 'fade-in 0.2s ease-out' }}>
      ...
      >
        ...
        <div className="flex items-center justify-between p-4">
          <button onClick={handleClose} className="p-2 rounded-full btn-glass haptic-press">
        ...
        {step === "mood" ? (
          ...
          <div className="px-4 pb-8 flex flex-col gap-4">
              ...
              <div className="flex gap-2">
                {hasImage && (
                  <div className="px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 text-xs text-primary flex items-center gap-1">
                    📷 Photo attached
                    <button onClick={() => setHasImage(false)} className="ml-1"><X className="w-3 h-3" /></button>
                  </div>
                )}
                {voiceSeconds > 0 && !isRecording && (
                  <div className="px-3 py-1.5 rounded-full bg-secondary/10 border border-secondary/20 text-xs text-secondary flex items-center gap-1">
                    ...
                    <button onClick={() => setVoiceSeconds(0)} className="ml-1"><X className="w-3 h-3" /></button>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => { setHasImage(true); toast({ title: "Photo added" }); }} className="p-2.5 rounded-full btn-glass haptic-press">
                <Image className="w-5 h-5 text-muted-foreground" />
              </button>
              <button onClick={toggleRecording} className={`p-2.5 rounded-full haptic-press ${isRecording ? "gradient-primary btn-glow" : "btn-glass"}`}>
                {isRecording ? <MicOff className="w-5 h-5 text-foreground" /> : <Mic className="w-5 h-5 text-muted-foreground" />}
              </button>
              <div className="flex-1" />
              <button onClick={() => setStep("mood")} className="px-3 py-1.5 rounded-full text-xs btn-glass text-muted-foreground haptic-press">
                Change mood
              </button>
            </div>
          </div>
